...
namespace jlib
{
	...
	public sealed class Parser
	{
		...
		public Parser(Type argumentSpecification, ErrorReporter reporter)
		{
			...
			{
				Debug.Assert(!argumentMap.ContainsKey(argument.LongName.ToLower()));
				this.argumentMap[argument.LongName.ToLower()] = argument;
				if (argument.ExplicitShortName)
				{
					if (argument.ShortName != null && argument.ShortName.Length > 0)
					{
						Debug.Assert(!argumentMap.ContainsKey(argument.ShortName.ToLower()));
						this.argumentMap[argument.ShortName.ToLower()] = argument;
					}
					else
					{
			...
			foreach (Argument argument in this.arguments)
			{
				if (!argument.ExplicitShortName)
				{
					if (argument.ShortName != null && argument.ShortName.Length > 0 && !argumentMap.ContainsKey(argument.ShortName.ToLower()))
						this.argumentMap[argument.ShortName.ToLower()] = argument;
					else
						argument.ClearShortName();
				}
					...
					if (argument.Length > 0)
					{
						switch (argument[0])
						{
							...
							case '/':
								...
								Argument arg = (Argument)this.argumentMap[option.ToLower()];
								if (arg == null)
								{
								...
								{
									if ((optionArgument == null || optionArgument.Length == 0) && i < args.Length - 1)
									{
										bool hadErrorNow = !arg.SetValue(optionArgument, destination, true);
										if (hadErrorNow)
										{
											i++;
											optionArgument = args[i];
											hadErrorNow = !arg.SetValue(optionArgument, destination);
										}
										hadError |= hadErrorNow;
									}
									else
									{
		...
		private class Argument
		{
			...
			{
				return SetValue(value, destination, false);
			}
			public bool SetValue(string value, object destination, bool suppressErrors)
			{
				...
				object newValue;
				if (!ParseValue(this.ValueType, value, out newValue, suppressErrors))
				{
					return false;
				}
				seenValue = true;
			...
			{
				return ParseValue(type, stringData, out value, false);
			}
			private bool ParseValue(Type type, string stringData, out object value, bool suppressErrors)
			{
				...
				if ((stringData != null || type == typeof(bool)) && (stringData == null || stringData.Length > 0))
				{
					try
					{
						...
						{
							value = byte.Parse(stringData);
							return true;
						}
						else if (type == typeof(sbyte))
						{
							value = sbyte.Parse(stringData);
							return true;
						}
						else if (type == typeof(long))
						{
							value = long.Parse(stringData);
							return true;
						}
						else if (type == typeof(ulong))
						{
							value = ulong.Parse(stringData);
							return true;
						}
						...
						{
							value = decimal.Parse(stringData);
							return true;
						}
						else if (type == typeof(char))
						{
							value = char.Parse(stringData);
							return true;
						}
						else if (type == typeof(System.Guid))
						{
							value = new System.Guid(stringData);
							return true;
						}
						else if (type == typeof(System.DateTime))
						{
							value = System.DateTime.Parse(stringData);
							return true;
						}
						else if (type == typeof(System.IO.FileInfo))
						{
							// should expand wildcards if this is a collection type...
							value = new System.IO.FileInfo(stringData);
							return true;
						}
						else
						{
			...
			public string SyntaxHelp
			{
				get
				{
					...
					else
					{
						...
						else if (valueType == typeof(System.IO.FileInfo))
						{
	...
	public class Util
	{
		...
		public static string[] GetFileListing(string filespec)
		{
			if (filespec == null || filespec.Length == 0) return new string[0];
			// If file has wildcards, then it's a list of files, otherwise it's one file
			string[] files;
			if (filespec.IndexOf("*") >= 0 || filespec.IndexOf("?") >= 0)
			{
